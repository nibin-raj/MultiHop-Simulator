...
# Schedulers
from .age_debt_scheduler import *
from .random_scheduler import *
...
#         print(ages)
#         print("AgeMeasurement:%u,%u,%u,%s" % (t, self.scheduler_id, self.nodeid, ages))
...
#             print('independent set', independent_sets)
            links = list(self.G_up.edges)
#             print('links',links)
...
#             print(current_time, last_received_packet, last_received_packet.generated_time)
            return packetageis
        else:
            return current_time + 1

class NetworkSimulator:
    def __init__(self, totalnode_num, link_list, source_list, commissioned_nodes, network_type, scheduler_id, scheduler, interference_model, interference_k_hop):
    ...
    def simulate_oneslot(self, t):
        ...
        if self.scheduler_id == SCHEDULER_RANDOM:
#             source_generatedpacket = self.scheduler.get_packet_generated_slot(t)
#             activation_vector_index = self.scheduler.get_activation_vector_slot(t)
        ...
        if not activation_vector_index == NOT_SELECTED:
            ...
            ps = []
            for p in self.network.G_up.nodes[0]["Node"].buffer:
                if len(p) > 0:
                    ps.append(p[0].sourceid)
                else:
                    ps.append(0)
